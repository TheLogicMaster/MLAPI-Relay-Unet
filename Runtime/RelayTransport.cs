...
namespace MLAPI.Relay
{
    public static class RelayTransport
    {
        ...
        {
            if (!UseRelay) return NetworkTransport.Connect(hostId, serverAddress, serverPort, exceptionConnectionId, out error);
        ...
        {
            if (!UseRelay) return NetworkTransport.ConnectWithSimulator(hostId, serverAddress, serverPort, exceptionConnectionId, out error, conf);
        ...
        {
            if (!UseRelay) return NetworkTransport.ConnectEndPoint(hostId, endPoint, exceptionConnectionId, out error);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddHost(topology);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddHost(topology, port);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddHost(topology, port, ip);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddHostWithSimulator(topology, minTimeout, maxTimeout);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddHostWithSimulator(topology, minTimeout, maxTimeout, port);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddWebsocketHost(topology, port);
        ...
        {
            if (!UseRelay) return NetworkTransport.AddWebsocketHost(topology, port, ip);
        ...
        {
            if (!UseRelay) return NetworkTransport.Disconnect(hostId, connectionId, out error);
        ...
        {
            if (!UseRelay) return NetworkTransport.Send(hostId, connectionId, channelId, buffer, size, out error);
            byte[] sendBuffer;
        ...
        {
            if (!UseRelay) return NetworkTransport.QueueMessageForSending(hostId, connectionId, channelId, buffer, size, out error);
        ...
        {
            if (!UseRelay) return NetworkTransport.ReceiveFromHost(hostId, out connectionId, out channelId, buffer, bufferSize, out receivedSize, out error);
        ...
        {
            if (!UseRelay) return NetworkTransport.Receive(out hostId, out connectionId, out channelId, buffer, bufferSize, out receivedSize, out error);
